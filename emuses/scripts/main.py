...
def add_output_folder_argument(parser):
    ...
    """
    parser.add_argument("output_folder", type=resolve_path, help="Output folder")
...
def add_input_dataset_optional_arguments(parser):
    ...
    parser.add_argument(
        "--arg_separator", default=",", help="Separator for the input dataset list"
    )
    parser.add_argument(
        "--input_header",
        default=None,
...
def add_scores_arguments(parser):
    ...
    parser.add_argument(
        "--scores_column", nargs="+", help="Column(s) for scores in the scores file"
    )
    parser.add_argument(
        "--classification",
        action="store_true",
...
def add_clustering_arguments(parser):
    ...
    parser.add_argument(
        "--min_cluster_size", type=int, default=5, help="Minimum cluster size"
    )
    parser.add_argument(
        "--interactive_plot",
        action="store_true",
...
def main():
    ...
    umap_parser = subparsers.add_parser(
        "umap", help="Train the UMAP and get the embeddings"
    )
    ...
    clustering_parser = subparsers.add_parser(
        "clustering", help="Perform clustering on embeddings"
    )
    ...
    clustering_parser.add_argument(
        "--load_embeddings", help="Path to precomputed embeddings"
    )
    ...
    heatmap_parser.add_argument(
        "--load_hdbscan", help="Path to a pre-trained HDBSCAN model"
    )
    heatmap_parser.add_argument(
    ...
    prediction_parser = subparsers.add_parser(
        "prediction", help="Train a prediction model"
    )
