...
def main():
    ...
    command = st.sidebar.selectbox(
        "Select Command", ["full", "umap", "clustering", "heatmap", "prediction"]
    )
    ...
    if command in ["full", "heatmap", "umap", "prediction"]:
        ...
        recursive_input_file_search = st.sidebar.checkbox(
            "Recursive Input File Search", value=False
        )
        args.recursive_input_file_search = recursive_input_file_search

        input_file_types = st.sidebar.text_input(
            "Input File Types (comma-separated)", value=""
        )
        args.input_file_types = (
            input_file_types.split(",") if input_file_types else None
        )
        ...
        test_size = st.sidebar.slider(
            "Test Size", min_value=0.0, max_value=1.0, value=0.2
        )
    ...
    if command in ["full", "heatmap", "prediction"]:
        ...
        scores_column = st.sidebar.text_input(
            "Scores Column(s) (comma-separated)", value=""
        )
    ...
    if command in ["full", "clustering", "heatmap"]:
        ...
        min_cluster_size = st.sidebar.number_input(
            "Minimum Cluster Size", min_value=2, value=5
        )
    ...
    if command in ["full", "heatmap"]:
        ...
        smoothing_option = st.sidebar.radio(
            "Smoothing Option", ["Default", "Sigma", "FWHM"]
        )
    ...
    def run_pipeline():
        try:
            ...
            pipeline.run(
                progress_callback=progress_callback, progress_queue=pipeline_queue
            )
                        ...
                        if isinstance(message, tuple):
                            ...
                            if message[0] == "plot":
                                ...
                                with plot_placeholder.container():
                                    ...
                                    st.plotly_chart(
                                        plot_obj
                                    )  # Display the new plot immediately
            ...
            if heatmap_plots:
                ...
                for tab, score_tag in zip(tabs, score_tags):
                    with tab:
                        ...
                        clusters_labels = [
                            clu for clu in score_tag_plots if clu != "clustering_plot"
                        ]
                        if clusters_labels:
                            cluster_tabs = st.tabs(
                                [f"Cluster {cluster}" for cluster in clusters_labels]
                            )
                            for cluster_tab, cluster in zip(
                                cluster_tabs, clusters_labels
                            ):
                                with cluster_tab:
                                    ...
                                    if isinstance(cluster_data, plt.Figure):
                                        ...
                                        cluster_data, plotly.graph_objects.Figure
                                    ):
    ...
    if not st.session_state.confirm_exit:
        if st.sidebar.button("Exit App", key="exit_button"):
            st.warning(
                "Are you sure you want to exit? Click 'Confirm Exit' to proceed."
            )
