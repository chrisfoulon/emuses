...
def process_column(args):
    ...
    try:
        ...
        if (
            len(filtered_data) < 3
            or len(other_data) < 3
            or np.all(filtered_data == 0)
            or np.all(other_data == 0)
        ):
...
def create_cluster_representative_maps(
    ...
    # plot array
    clusters, indices_list = separate_clusters_and_extract_coords(
        array, discrete_embeddings
    )
    ...
    for cluster, indices in zip(clusters, indices_list):
        ...
        stat_map, pval_map, effect_size_map = input_matrix_stat_map(
            input_matrix, indices, test_name
        )
...
def train_model(
    ...
    if categorical:
        ...
        if num_removed > 0:
            print(
                f"Warning: {num_removed} test observations removed for score {score_name} because they have no value."
            )
        ...
        best_permutation = max(
            permutation_metrics, key=lambda x: np.mean(x["accuracy_scores_val"])
        )
        ...
        print(
            f'{score_name} - Avg Training Accuracy: {np.mean(best_permutation["accuracy_scores_train"]):.2f}'
        )
        ...
        for t in range(n_targets):
            ...
            if num_removed > 0:
                print(
                    f"Warning: {num_removed} test observations removed for target column {t} in score {score_name} because they have no value."
                )
            ...
            for perm in range(num_permutations):
                ...
                for train_index, val_index in kf.split(train_coords):
                    ...
                    range_of_values_t = (
                        max_score_t - min_score_t if max_score_t != min_score_t else 1
                    )
                    mae_max_val = (
                        (mae_val / max_score_t) * 100 if max_score_t != 0 else 0
                    )
                    ...
                    normalized_mae_train = (
                        mean_absolute_error(y_train_cv, y_train_pred)
                        / range_of_values_t
                    ) * 100
                    normalized_mse_train = (
                        mean_squared_error(y_train_cv, y_train_pred)
                        / (range_of_values_t**2)
                    ) * 100
                    mae_max_train = (
                        (mean_absolute_error(y_train_cv, y_train_pred) / max_score_t)
                        * 100
                        if max_score_t != 0
                        else 0
                    )
            ...
            best_permutation = max(
                permutation_metrics, key=lambda x: np.mean(x["r2_scores_val"])
            )
            best_models = best_permutation["models"]
            ...
            max_score_t = np.max(y_train_t)
            range_of_values_t = (
                max_score_t - min_score_t if max_score_t != max_score_t else 1
            )
            ...
            print(
                f"Target {t} - Avg Training R²: {np.mean(best_permutation['r2_scores_train']):.2f}"
            )
            ...
            metrics_df = pd.DataFrame(
                {
                    ...
                    "Normalized Validation MSE (%)": best_permutation[
                        "normalized_mse_val_list"
                    ],
                    "MAE_max%": best_permutation["mae_max_scores"],
                }
            )
            metrics_df["Normalized MAE (%)"] = best_permutation[
                "normalized_mae_train_list"
            ]
            metrics_df.loc[k, "Fold"] = "Avg Training"
            metrics_df.loc[k, "Training R²"] = np.mean(
                best_permutation["r2_scores_train"]
            )
            metrics_df.loc[k, "Normalized Validation MSE (%)"] = np.mean(
                best_permutation["normalized_mse_train_list"]
            )
            metrics_df.loc[k, "MAE_max%"] = np.mean(
                best_permutation["mae_max_train_list"]
            )
            metrics_df.loc[k, "Normalized MAE (%)"] = np.mean(
                best_permutation["normalized_mae_train_list"]
            )
            ...
            metrics_df.to_excel(
                os.path.join(
                    ...
                    f"{score_name}_target_{t}_validation_metrics.xlsx",
                ),
                index=False,
            )
        ...
        if train_scores.shape[1] == 1:
            ...
            for t in range(n_targets):
                ...
                plt.plot(
                    [test_scores[:, t].min(), test_scores[:, t].max()],
                    [test_scores[:, t].min(), test_scores[:, t].max()],
                    "k--",
                    lw=2,
                )
                ...
                plt.ylabel("Predicted Scores")
                plt.title(
                    f"Target {t} - Actual vs Predicted Scores\nR² = {r2_test_dict[t]:.2f}"
                )
                plt.savefig(
                    str(
                        Path(output_folder)
                        / f"{score_name}_target_{t}_prediction_plot.png"
                    )
                )
                if show_plot:
                    plt.show()
                ...
                plt.savefig(
                    str(
                        Path(output_folder)
                        / f"{score_name}_target_{t}_correlation_plot.png"
                    )
                )
                if show_plot:
                    plt.show()
...
def train_model_kriging(training_df, test_df, score_name, output_folder):
    ...
    for train_index, val_index in kf.split(train_coords):
        ...
        # Normalize errors
        normalized_mae_train = (
            mean_absolute_error(y_train, y_train_pred) / range_of_values
        ) * 100
        normalized_mse_train = (
            mean_squared_error(y_train, y_train_pred) / (range_of_values**2)
        ) * 100
    ...
    for i, batch_models in enumerate(models):
        for j, model in enumerate(batch_models):
            model_path = os.path.join(
                output_folder, f"{score_name}_model_fold_{i}_batch_{j}.pkl"
            )
    ...
    # Save to Excel
    metrics_df.to_excel(
        os.path.join(output_folder, f"{score_name}_validation_metrics.xlsx"),
        index=False,
    )
...
# TODO UNUSED FOR NOW
def train_model_xgboost_with_fold_grid_search(
    training_df, test_df, score_name, output_folder, augmentation_factor=1
):
    os.makedirs(output_folder, exist_ok=True)
    ...
    for train_index, val_index in kf.split(train_coords):
        ...
        r2_scores_val.append(r2_val)
        r2_scores_train.append(r2_train)
        normalized_mae_train_list.append(
            (mean_absolute_error(y_train, y_train_pred) / range_of_values) * 100
        )
        normalized_mse_train_list.append(
            (mean_squared_error(y_train, y_train_pred) / (range_of_values**2)) * 100
        )
        mae_max_train_list.append(
            (mean_absolute_error(y_train, y_train_pred) / max_score) * 100
        )
...
def compute_all_gwd(embeddings, sigma):
    ...
    for i in range(n):
        gwd_matrix[i, :] = compute_gwd_for_point(
            embeddings, embeddings[i : i + 1], sigma
        )
    return gwd_matrix
...
def compute_gwd_summary(embeddings, sigma, mode="basic"):
    ...
    for i in range(n_points):
        ...
        weighted_vars[i] = np.sum(weights * (distances - weighted_mean) ** 2) / (
            np.sum(weights) + 1e-8
        )
    ...
    if mode == "basic":
        ...
        raise ValueError(
            f"Mode {mode} is not supported. Choose 'basic', 'extended', or 'full'."
        )
...
def train_predictive_model_gpy(
    combined_features,
    target,
    output_folder,
    mode="regression",
    ...
    for train_idx, val_idx in cv.split(combined_features):
        ...
        if (sparse_threshold is not None) and (X_train.shape[0] > sparse_threshold):
            ...
            if mode == "regression":
                ...
                inducing = X_train[
                    np.random.choice(
                        np.arange(X_train.shape[0]), size=num_inducing, replace=False
                    )
                ]
            ...
            if mode == "regression":
                model = GPy.models.GPRegression(
                    X_train, y_train, kernel=GPy.kern.RBF(input_dim=X_train.shape[1])
                )
            else:
        ...
        if mode == "regression":
            ...
            fold_pred = {
                "predictions": y_pred_mean.flatten(),
            ...
            uncertainties = np.array(
                [entropy([p, 1 - p], base=2) for p in y_pred_prob.flatten()]
            )
            fold_pred = {
    ...
    if (sparse_threshold is not None) and (
        combined_features.shape[0] > sparse_threshold
    ):
        inducing = combined_features[
            np.random.choice(
                np.arange(combined_features.shape[0]), size=num_inducing, replace=False
            )
        ]
        if mode == "regression":
            final_model = GPy.models.SparseGPRegression(
                combined_features,
                y_full,
            ...
            final_model = GPy.models.SparseGPClassification(
                combined_features,
                y_full,
                kernel=GPy.kern.RBF(input_dim=combined_features.shape[1]),
                Z=inducing,
        ...
        if mode == "regression":
            final_model = GPy.models.GPRegression(
                combined_features,
                y_full,
                kernel=GPy.kern.RBF(input_dim=combined_features.shape[1]),
            )
        else:
            final_model = GPy.models.GPClassification(
                combined_features,
                y_full,
                kernel=GPy.kern.RBF(input_dim=combined_features.shape[1]),
            )
    ...
    if mode == "regression":
        ...
        full_predictions = {
            "predictions": full_mean.flatten(),
            "uncertainty": full_std.flatten(),
        }
    else:
        full_prob, _ = final_model.predict(combined_features)
        full_pred = (full_prob >= 0.5).astype(int)
        full_uncertainty = np.array(
            [entropy([p, 1 - p], base=2) for p in full_prob.flatten()]
        )
        full_predictions = {
            "predictions": full_pred.flatten(),
            "probabilities": full_prob.flatten(),
    ...
    if mode == "regression":
        plt.errorbar(
            np.arange(len(full_predictions["predictions"])),
            full_predictions["predictions"],
            yerr=full_predictions["uncertainty"],
            fmt="o",
            alpha=0.5,
        )
...
def new_pipeline_test(
    ...
    sigma_values = np.linspace(
        best_params["sigma"] * 0.5, best_params["sigma"] * 1.5, num=20
    )
    ...
    plt.axvline(
        final_sigma, color="red", linestyle="--", label=f"Final σ: {final_sigma:.5f}"
    )
    ...
    features_4 = np.hstack(
        (embeddings, gwd_summaries)
    )  # [embeddings (2) + GWD summaries (2)]
    features_3 = np.hstack(
        (embeddings, gwd_summaries[:, :1])
    )  # [embeddings (2) + first summary (1)]
    ...
    feature_sets = {
        "combined_features": (
            features_4,
            "Combined Features (Embeddings + GWD summaries)",
        ),
        ...
        "pca_gwd": (
            pca_gwd_features,
            f"PCA on Full GWD matrix [{pca.n_components_} components]",
        ),
    }
    ...
    if test_embeddings is not None and test_labels is not None:
        ...
        test_gwd_summaries = compute_gwd_summary_test(
            test_embeddings, embeddings, final_sigma, mode="basic"
        )
    ...
    def optimize_train_model(
        feature_set_name, X_train, y_train, X_test=None, y_test=None
    ):
        ...
        if X_test is not None and y_test is not None:
            ...
            test_results = {
                ...
                "test_mae": test_mae,
                "normalized_mse": (
                    (test_mse / (global_range**2)) * 100
                    if global_range != 0
                    else test_mse
                ),
                "normalized_mae": (
                    (test_mae / global_range) * 100 if global_range != 0 else test_mae
                ),
            }
            ...
            plt.title(
                f'{feature_set_name} - {results["best_model_name"]}\nTest R²: {test_r2:.4f}'
            )
            plt.savefig(
                os.path.join(
                    feature_set_dir,
                    f'test_predictions_{results["best_model_name"]}.png',
                )
            )
            plt.close()
        ...
        model_file = os.path.join(
            feature_set_dir, f'best_model_{results["best_model_name"]}.joblib'
        )
    ...
    summary_df.sort_values(
        by=(
            "Test R²"
            if "Test R²" in summary_df.columns
            and any(x != "N/A" for x in summary_df["Test R²"])
            else "CV R²"
        ),
        ascending=False,
        inplace=True,
    ...
    if test_feature_sets:
        ...
        best_fs = max(
            summary.items(),
            key=lambda x: (
                x[1]["test_metrics"].get("test_r2", -float("inf"))
                if x[1]["test_metrics"]
                else -float("inf")
            ),
        )[0]
    ...
    if test_feature_sets and any(
        ...
        bars = plt.bar(
            ...
            [
                (
                    summary[fs_name]["test_metrics"]["test_r2"]
                    if summary[fs_name]["test_metrics"]
                    else 0
                )
...
def optuna_model_selection(
    ...
    def objective(trial):
        ...
        if model_type == "gp":
            ...
            kernel_option = trial.suggest_categorical(
                "kernel_option", ["rbf", "matern"]
            )
            ...
            if use_constant:
                constant_value = trial.suggest_float(
                    "constant_value", 0.1, 10.0, log=True
                )
            ...
            kernel_type = trial.suggest_categorical(
                "kernel_type", ["gaussian", "epanechnikov", "triangular"]
            )
    ...
    if best_model_type == "gp":
        ...
        best_model = KernelRegressor(
            kernel=best_params["kernel_type"], sigma=best_params["sigma"]
        )
    ...
    if output_folder:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_folder, f"optimization_history_{best_model_type}.png")
        )
        plt.close()
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(output_folder, f"param_importances_{best_model_type}.png")
            )
        except Exception as e:
        ...
        if len(models) > 1:
            ...
            plt.boxplot(
                box_data,
                labels=[model for model in model_scores.keys() if model_scores[model]],
            )
            plt.title(f"Model Comparison for {feature_set_name}")
            plt.ylabel(
                "Score (higher is better)"
                if direction == "maximize"
                else "Score (lower is better)"
            )
...
def compute_all_gwd(embeddings, sigma, inference_mode=False, reference_embeddings=None):
    """
    ...
    Parameters:
        embeddings (np.ndarray): Array of shape (n_samples, n_features)
        sigma (float): The Gaussian kernel bandwidth
        inference_mode (bool): If True, compute GWD relative to reference_embeddings
        reference_embeddings (np.ndarray): Reference embeddings for inference mode.
                                          If None and inference_mode=True, uses embeddings.
    ...
    if inference_mode and reference_embeddings is not None:
        ...
        # Standard mode: compute pairwise distances within embeddings
...
def compute_gwd_summary(
    embeddings, sigma, mode="basic", inference_mode=False, reference_embeddings=None
):
    """
    ...
    if inference_mode and reference_embeddings is not None:
        # Use the test function for inference mode
        return compute_gwd_summary_test(embeddings, reference_embeddings, sigma, mode)
    else:
        # Compute the full GWD matrix
        gwd_matrix = compute_all_gwd(
            embeddings, sigma, inference_mode=False, reference_embeddings=None
        )
        ...
        if mode == "basic":
            ...
            return np.column_stack(
                (ess, entropy, mean_dist, median_dist, max_dist, min_dist, std_dist)
            )
    ...
    if mode == "basic":
        ...
        for i in range(n_test):
            ...
            distances = np.linalg.norm(
                train_embeddings - test_embeddings[i : i + 1], axis=1
            )
            weights = gwd_matrix[i, :]
