...
def nested_cv_kernel_regression(X, y, sigma_values, n_outer=5, n_inner=5, classification=False):
    ...
    for train_index, test_index in outer_kf.split(X):
        ...
        # Optionally, evaluate on X_test_outer here.
...
def run_kernel_heatmap_analysis(
    ...
    For each score tag in `scores_vectors_dict`, the function:
      ...
      5. Forms a combined heatmap by subtracting (uncertainty_penalty × standard deviation)
         from the mean prediction to penalize regions with high ensemble uncertainty.
      6. Optionally computes effect size maps for high-confidence points (those with predictions above a threshold).
         For continuous targets, the threshold is determined dynamically based on a normality test:
         if the training predictions are normally distributed, the threshold is set at mean + 2*std;
    ...
    uncertainty_penalty : float, default=0.5
        Multiplier to penalize regions with high ensemble prediction uncertainty when computing the combined heatmap.
    input_type : str, default='image'
        The type of input data used for effect size analysis (e.g., 'image', 'nifti', 'spreadsheet').
    classification : bool, default=False
        If True, uses a kernel logistic regressor for binary classification; otherwise, uses a kernel regressor
    ...
    effect_size_test : str, default='mann-whitney'
        The statistical test to use for computing effect size maps (e.g., 'mann-whitney').
    highlight_points : bool, default=True
        If True, the original embedding points will be highlighted on the generated heatmap plots.
    ...
    generate_plots : bool, default=False
        ...
        the plot objects are returned in the output.
    output_format_info : any, optional
        Additional information required for formatting the output (e.g., an affine matrix for NIfTI images,
        a target output shape for image data, or column names for spreadsheets).

    Returns
    -------
    heatmap_dict : dict
        Dictionary mapping each score tag (str) to a dictionary with the following keys:
          'mean_heatmap' : np.ndarray
              The mean predictions over the grid, reshaped to (grid_size, grid_size).
          'std_heatmap' : np.ndarray
              The standard deviation of predictions over the grid, reshaped to (grid_size, grid_size).
          'combined_heatmap' : np.ndarray
              The combined heatmap computed as (mean_heatmap - uncertainty_penalty * std_heatmap).
          'grid_x' : np.ndarray
              The x-coordinates of the grid.
          'grid_y' : np.ndarray
              The y-coordinates of the grid.
          'models' : list
              List of trained kernel regression (or logistic regression) models obtained via nested cross-validation.
          'effect_size' : dict
              Dictionary mapping cluster labels (or indices) to computed effect size maps for high-confidence regions.
          'plot' : matplotlib.figure.Figure or None
              The generated plot object if `generate_plots` is True; otherwise, None.
    """
    ...
    for score_tag, y in scores_vectors_dict.items():
        ...
        # Run nested CV to get models.
        models = nested_cv_kernel_regression(
        ...
        # Compute effect size maps.
        # First, compute ensemble predictions on the training embeddings.
        train_pred_mean, _ = ensemble_predict(models, embeddings)

        # For continuous outcomes, determine a dynamic threshold based on normality.
        ...
        heatmap_dict[score_tag] = {
            ...
            'grid_y': grid_y,
            'models': models,
            'effect_size': effect_size_maps if 'effect_size_maps' in locals() else {},
            'plot': plot_obj
        }
    return heatmap_dict
