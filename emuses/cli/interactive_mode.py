...
class WorkflowPrompt:
    ...
    def prompt_text(self, message: str, default: Optional[str] = None) -> str:
        ...
        self.prompt_history.append(
            {"type": "text", "message": message, "result": result, "default": default}
        )
...
class ParameterValidator:
    ...
    def validate_file_path(self, path: str) -> ValidationResult:
        ...
        if not os.path.exists(path):
            return ValidationResult(
                is_valid=False, error_message=f"Path does not exist: {path}"
            )

        # Check if path is accessible
        if not os.access(path, os.R_OK):
            return ValidationResult(
                is_valid=False, error_message=f"Path is not readable: {path}"
            )
    ...
    def validate_numeric(
        ...
        try:
            if isinstance(value, str):
                ...
                return ValidationResult(
                    is_valid=False, error_message=f"Value is not numeric: {value}"
                )
        except (ValueError, TypeError):
            return ValidationResult(
                is_valid=False, error_message=f"Cannot convert to numeric: {value}"
            )
        ...
        return ValidationResult(is_valid=True, value=numeric_value)
    ...
    def validate_string(
        ...
        if not isinstance(value, str):
            return ValidationResult(
                is_valid=False, error_message=f"Value is not a string: {value}"
            )
...
class SecureFilePicker:
    ...
    def select_file(
        self, prompt: str, allowed_extensions: Optional[List[str]] = None
    ) -> str:
        """
    ...
    def _validate_file_selection(
        self, file_path: str, allowed_extensions: List[str]
    ) -> bool:
        """
    ...
    def is_file_size_valid(
        self, file_path: str, max_size_mb: Optional[float] = None
    ) -> bool:
        """
    ...
    def check_file_permissions(self, file_path: str, access_type: str) -> bool:
        ...
        """
        access_map = {"read": os.R_OK, "write": os.W_OK, "execute": os.X_OK}
    ...
    def save_template(self, template_data: Dict[str, Any]) -> bool:
        ...
        if self.template_directory:
            ...
            template_path = os.path.join(
                self.template_directory, f"{template_name}.json"
            )
...
class InteractiveParameterReview:
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validated_count": sum(
                1 for p in self.parameters.values() if p["validated"]
            ),
