...
def status(
    ...
    try:
        ...
        # Get location info for better visibility
        registry_path = info.get('registry_path', str(registry.registry_path))
        default_path = Path.home() / ".emuses" / "model_registry"
        is_default = Path(registry_path) == default_path

        console.print("\n[bold cyan]Model Registry Status[/bold cyan]")

        # Display registry location prominently
        location_type = "[yellow]Default Location[/yellow]" if is_default else "[blue]Custom Location[/blue]"
        console.print(f"Location: {location_type}")
        console.print(f"Path: [bold]{registry_path}[/bold]")
        if "/.emuses/" in registry_path:
            console.print("[dim]💡 Hidden directory - access with file manager or terminal[/dim]")

        console.print(f"\nVersion: [yellow]{info.get('version', 'Unknown')}[/yellow]")
...
def storage(
    ...
    try:
        ...
        info = enhanced_info["basic_info"]
        location_info = enhanced_info["registry_location"]

        # Display registry location prominently
        console.print(f"\n[bold cyan]Registry Location[/bold cyan]")
        location_type = "[yellow]Default[/yellow]" if location_info["is_default"] else "[blue]Custom[/blue]"
        console.print(f"Type: {location_type}")
        console.print(f"Path: [bold]{location_info['path']}[/bold]")
        if location_info["hidden_directory"]:
            console.print(f"[dim]💡 This is a hidden directory (~/.emuses/)[/dim]")
        if not location_info["exists"]:
            console.print(f"[yellow]⚠️  Registry directory will be created when needed[/yellow]")
        ...
        # Display model breakdown if there are models
        model_breakdown = enhanced_info["model_breakdown"]
        if model_breakdown:
            console.print(f"\n[bold cyan]Model Storage Breakdown[/bold cyan]")

            # Sort models by size (largest first)
            sorted_models = sorted(model_breakdown.items(),
                                 key=lambda x: x[1]["size_mb"], reverse=True)

            # Show top 10 models or all if fewer than 10
            models_to_show = sorted_models[:10]

            for model_id, model_info in models_to_show:
                size_mb = model_info["size_mb"]
                file_count = len(model_info["files"])

                if size_mb >= 1:
                    size_display = f"[green]{size_mb:.1f} MB[/green]"
                else:
                    size_display = f"[dim]{size_mb:.3f} MB[/dim]"

                console.print(f"  • {model_id}: {size_display} ({file_count} files)")

            if len(sorted_models) > 10:
                console.print(f"  [dim]... and {len(sorted_models) - 10} more models[/dim]")

        # Display optimization suggestions
        suggestions = enhanced_info["optimization_suggestions"]
        if suggestions:
            console.print(f"\n[bold yellow]Storage Optimization Suggestions[/bold yellow]")
