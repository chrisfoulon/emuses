...
def run_pipeline_locally(
    ...
    """Run a pipeline in *this* process, with no job manager and no service.

    ADR §4 defines local mode as "CLI, file-based storage, in-process execution", but the
    CLI used to fork a FastAPI service and submit its own job over HTTP. This is the entry
    point that makes the CLI match the architecture: the same
    ``_run_pipeline_in_process`` the service uses, called directly.

    ``job_manager`` is ``None`` rather than a stub. ``_run_pipeline_in_process`` touches
    only ``_context_to_emuses_args``, ``_create_emuses_progress_adapter``, ``logger`` and
    ``_merge_pipeline_context`` - never ``job_manager`` - so there is nothing for a stub to
    stand in for, and a stub would quietly absorb a future call that ought to fail loudly.
    ``tests/test_local_execution.py`` asserts that property rather than leaving it as a
    comment that can go stale.

    Parameters
    ----------
    context : dict
        Pipeline context in the shape the endpoints build: ``{"config": ...,
        "input_dataset": ..., "scores_dataset": ...}``.
    memory_limit_ratio : float
        Passed through to the resource monitor.
    progress_callback : callable, optional
        Receives ``(stage_name, progress, message)``.
    ...
    """
    runner = PipelineRunner(job_manager=None)
    return runner._run_pipeline_in_process(
        context, memory_limit_ratio, progress_callback
    )
