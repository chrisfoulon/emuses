...
class BackgroundTaskManager:
    ...
    def _start_background_threads(self) -> None:
        ...
        if self._cleanup_thread is None or not self._cleanup_thread.is_alive():
            self._cleanup_thread = threading.Thread(
                target=self._cleanup_worker, daemon=True, name="TaskCleanupWorker"
            )
    ...
    def create_task(
        ...
        if db_session is not None and (
            expected_compute_hours > 0 or expected_storage_gb > 0
        ):
    ...
    def submit_task(self, task_id: UUID) -> bool:
        ...
        with self._task_lock:
            ...
            future.add_done_callback(lambda fut: self._on_task_completed(task_id, fut))
...
def _execute_pipeline_task(
    ...
    try:
        ...
        executor = PipelineRunner(
            input_dir=job_dir / "input", output_dir=job_dir / "output"
        )
