...
def initialize_task_manager(
    ...
    if _global_task_manager is not None:
        logger.warning(
            "Task manager already initialized, shutting down previous instance"
        )
...
async def task_manager_lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        # Initialize task manager (initialization has side effects for global state)
...
def get_task_manager_health() -> dict:
    ...
    try:
        ...
        healthy = (
            system_status["executor_active"]
            and system_status["active_workers"] <= system_status["max_workers"]
        )
