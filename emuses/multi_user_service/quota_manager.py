...
from emuses.multi_user_service.models import TrainingJob, User
...
class QuotaManager:
    ...
    def validate_concurrent_job_limit(
        self, db: Session, user_id: UUID, limit: Optional[int] = None
        ...
        if is_valid:
            ...
            message = (
                f"User has reached maximum concurrent jobs ({current_jobs}/{limit})"
            )

        return ConcurrentJobValidationResult(
            is_valid=is_valid, current_jobs=current_jobs, limit=limit, message=message
        )
    ...
    def get_user_storage_usage(self, db: Session, user_id: UUID) -> Dict[str, float]:
        ...
        if not user:
            return {"used_gb": 0.0, "quota_gb": 0.0, "percentage": 0.0}

        percentage = (
            (user.storage_used_gb / user.storage_quota_gb * 100)
    ...
    def validate_storage_quota(
        self, db: Session, user_id: UUID, additional_gb: float = 0.0
        ...
        if not user:
            return QuotaValidationResult(
                is_valid=False, current_usage=0.0, limit=0.0, message="User not found"
            )
    ...
    def get_user_compute_usage(self, db: Session, user_id: UUID) -> Dict[str, float]:
        ...
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            return {"used_hours": 0.0, "quota_hours": 0.0, "percentage": 0.0}

        percentage = (
            (user.compute_used_hours / user.compute_quota_hours * 100)
            if user.compute_quota_hours > 0
            else 0.0
        )
    ...
    def validate_compute_quota(
        self, db: Session, user_id: UUID, additional_hours: float = 0.0
    ) -> QuotaValidationResult:
    ...
    def validate_all_quotas(
        ...
        compute_result = self.validate_compute_quota(
            db, user_id, additional_compute_hours
        )
    ...
    def update_user_storage_usage(
        self, db: Session, user_id: UUID, storage_delta_gb: float
    ) -> bool:
    ...
    def update_user_compute_usage(
        self, db: Session, user_id: UUID, compute_delta_hours: float
    ) -> bool:
    ...
    def get_user_quota_summary(self, db: Session, user_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "concurrent_jobs": {
                ...
                "limit": self.default_concurrent_job_limit,
                "percentage": (
                    concurrent_jobs / self.default_concurrent_job_limit * 100
                ),
            },
    ...
    def get_users_near_quota_limit(
        ...
        for user in users:
            ...
            if (
                storage_percent >= storage_threshold_percent
                or compute_percent >= compute_threshold_percent
            ):
                near_limit_users.append(
                    {
                        ...
                        "over_storage_limit": storage_percent
                        >= storage_threshold_percent,
                        "over_compute_limit": compute_percent
