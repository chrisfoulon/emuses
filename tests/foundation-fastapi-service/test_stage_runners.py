...
class TestResourceMonitor:
    ...
    def test_resource_monitor_initialization(self):
        ...
        assert monitor.memory_limit_bytes == 8.0 * 1024 * 1024 * 1024  # 8GB
    ...
    def test_resource_monitor_custom_limits(self):
        ...
        monitor = ResourceMonitor(memory_limit_gb=4.0, cpu_percent_limit=80.0)
        assert monitor.memory_limit_bytes == 4.0 * 1024 * 1024 * 1024  # 4GB
    ...
    def test_resource_monitor_check_resources_ok(self, mock_cpu, mock_memory):
        ...
        # Mock memory usage (4GB used, 8GB limit)
        mock_memory.return_value = Mock(used=4 * 1024 * 1024 * 1024)
        mock_cpu.return_value = 50.0  # 50% CPU usage
        
        monitor = ResourceMonitor(memory_limit_gb=8.0, cpu_percent_limit=90.0)
        monitor.start_monitoring()
    ...
    def test_resource_monitor_check_resources_memory_exceeded(self, mock_cpu, mock_memory):
        ...
        # Mock memory usage (10GB used, 8GB limit)
        mock_memory.return_value = Mock(used=10 * 1024 * 1024 * 1024)
        mock_cpu.return_value = 50.0  # 50% CPU usage
...
class TestBaseStageRunner:
    ...
    def test_validate_parameters_success(self):
        ...
        # Should not raise exception
        runner._validate_parameters(config, parameter_ranges)

    def test_validate_parameters_out_of_range(self):
        """Test _validate_parameters with out-of-range parameters"""
        job_manager = Mock(spec=JobManager)
        runner = BaseStageRunner(job_manager)
        
        config = Mock()
        config.param1 = 15  # Outside range [1, 10]
        
        parameter_ranges = {
            "param1": (1, 10)
        }
        
        with pytest.raises(ValueError, match="Parameter param1=15 outside valid range"):
            runner._validate_parameters(config, parameter_ranges)
...
class TestUMAPStageRunner:
    ...
    async def test_umap_stage_runner_invalid_parameters(self, umap_runner):
        ...
        config = Mock()
        config.n_components = 100  # Outside valid range [2, 50]
        config.n_neighbors = 15
        config.min_dist = 0.1
        config.min_cluster_size = 10
        ...
        with pytest.raises(ValueError, match="Parameter n_components=100 outside valid range"):
...
class TestHeatmapStageRunner:
    ...
    async def test_heatmap_stage_runner_invalid_parameters(self, heatmap_runner):
        ...
        config = Mock()
        config.cv_folds = 25  # Outside valid range [2, 20]
        config.test_size = 0.2
        config.max_iter = 1000
        ...
        with pytest.raises(ValueError, match="Parameter cv_folds=25 outside valid range"):
