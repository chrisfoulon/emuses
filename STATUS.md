...
00c. [ ] **BBS replaces DSD_repro as the real-data cohort — decided 2026-09-14, plan written
    ...
    **Run 0 is small on purpose (Chris, 2026-09-16):** the method is not yet trusted (prediction
    ...
    the clinical database's linkage/verification lists as source of truth. **Morphospace:** single-
    dataset mode, trained once on 80 % of the 331 on the lab compute node and reused via
    `--load_umap` for every target set; reuse verified on public data (B10), valid only with the
    identical file list in both commands (plan §5). PR #19 merged (B1). **Blocking before run 0:**
    search space `hard` vs `disconnectome` (B8 — `hard` is not what it was remembered as); trial
    budget from measured cost (B9); labels CSV built through the validated lookup, indexed by full
    subject string, all 331 rows (B5); compute-node SSH access (key refused 2026-09-16). Floor/permutation/MDE run alongside by local script (B3), then go into
