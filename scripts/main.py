# Main script for EMUSES pipeline integrating full functionality
import argparse
import os
...
from tools.clustering_utils import load_hdbscan_model, save_hdbscan_model
from tools.inputs_utils import (detect_dataset_type, \
                                process_images, nifti_dataset_to_matrix,
...
from tools.clustering_utils import cluster_coordinates


"""
Main functions composing the "full" EMUSES pipeline:
'umap': 
    train_and_save_umap_and_embeddings: 
        - input_matrix
        - output_folder
        - prefix [optional]
        - kwarg (dict of args passed to UMAP) [optional]
'clustering':
    cluster_coordinates:
        - training_embeddings
        - min_cluster_size [optional]
'heatmap':
    run_heatmap_analysis:
        ...
        - grid_size [optional]
        - sigma [optional]
        - correlation_threshold [optional]
        - highlight_points [optional]
'prediction':
    train_and_test_model_per_label:
        ...
        - test_labels
        - output_folder
Some parameters influence these main parameters so the list is not exhaustive.
"""
...
def main():
    ...
    # output_format_info
    full_parser.add_argument('--output_format_info', help='Information needed to format the output')
    ...
    # TODO we need the embeddings but they could be transformed from the UMAP model so we need to add that option
    clustering_parser.add_argument('--load_embeddings',
    ...
    prediction_parser.add_argument('input_dataset', type=str,
                             help='Input dataset of either images (jpg), NIfTI, or MNIST')
    prediction_parser.add_argument('--load_umap', help='Path to a pre-trained UMAP model')
    # TODO we need the embeddings but they could be transformed from the UMAP model so we need to add that option
    prediction_parser.add_argument('--load_embeddings',
                                   help='Path to precomputed embeddings')
    prediction_parser.add_argument('--scores', help='Path to the scores associated with the embeddings')
    prediction_parser.add_argument('--classification', action='store_true',
                                   help='The scores are only one column with multiple integer values (classes)')
    prediction_parser.add_argument('--test_size', type=float, default=0.2,
                             help='Test size for splitting the dataset')

    args = parser.parse_args()

    # print the arguments (Separate the None and the others)
    print("Arguments:")
    none_args = {k: v for k, v in vars(args).items() if v is None}
    # now go through the args, print the non-None ones and store the None ones
    for k, v in vars(args).items():
        if v is not None:
            print(f"{k}: {v}")

    print("None arguments:")
    for k, v in none_args.items():
        print(f"{k}: {v}")
    ...
    if not output_folder.is_dir():
        raise ValueError(f"Output folder {output_folder} is not a valid path")

    # Save the arguments to a log file
    os.makedirs(output_folder / 'log', exist_ok=True)
    dict_args = vars(args)
    dict_args['datetime'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    save_json(output_folder / 'log' / f'arguments_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.json', dict_args)
    ...
    elif dataset_type == 'input_matrix':
    ...
    if args.command == 'umap' and '--test_size' not in args:
        ...
        if 'test_size' in args:
            ...
            # TODO make sure it does not cause bugs
            test_size = 0.0
    if test_size > 0:
        ...
        train_features = input_matrix
        train_labels = scores

    # Load pre-trained UMAP model if provided
    if args.command in ['full', 'heatmap', 'clustering', 'prediction'] and 'load_umap' in args and args.load_umap:
        umap_model_path = Path(args.load_umap).resolve()
        trained_umap, _ = load_umap_model(umap_model_path)
    elif args.command in ['full', 'umap', 'prediction']:
        # Train UMAP
        trained_umap, embeddings, umap_path, embeddings_path, input_matrix_path = train_and_save_umap_and_embeddings(
            train_features, output_folder, pref=args.prefix)
    ...
    if args.command in ['full', 'clustering']:
        if args.load_hdbscan:
            ...
            save_json(output_folder / 'cluster_labels.json', cluster_labels.tolist())
            # Save the HDBSCAN model
            save_hdbscan_model(clusterer, output_folder, prefix=args.prefix)
        ...
        if args.interactive_plot:
            plot_clustering_interactive_with_hover(training_embeddings, cluster_labels,
                                                   output_path=output_folder / 'clustering_plot.html',
                                                   show_plot=True,
                                                   return_plot=False)
    ...
    if args.command in ['full', 'heatmap']:
        ...
        if clusterer is None and not args.load_hdbscan:
    ...
    if args.command in ['full', 'prediction']:
        ...
        if scores is None and args.scores is None:
            raise ValueError("Scores are required for prediction.")

        # TODO if test_embeddings is None, or test_df is None we need to throw an error

        if test_df is None or test_embeddings is None:
            raise NotImplementedError("Empty test sets are not yet supported.")
        # TODO train_and_test_model_per_label only works with classification for now (it looks at the unique values in the scores)
        train_and_test_model_per_label(
