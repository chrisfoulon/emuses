...
**When to use**: Comprehensive spatial analysis, brain-behavior correlation, statistical effect mapping

**Complete Analysis** (includes all analysis types):
```bash
# Full statistical analysis with all outputs
emuses full comprehensive_analysis/ brain_features.csv \
  --scores behavioral_data.csv

# Generates complete analysis structure:
# - prediction-heatmaps/: Model predictions across embedding space
# - correlation-heatmaps/: UMAP correlation with behavioral scores  
...
# - heatmap_visualizations/: Base heatmap plots with scatter overlays
```

**Analysis Output Exploration**:
```bash
# View analysis structure
ls analysis_output/target_0/
# prediction-heatmaps/  correlation-heatmaps/  prediction-effects/  
# correlation-effects/   heatmap_visualizations/  interactive_plots/

# Load spatial grids for analysis
import numpy as np
prediction_grid = np.load('target_0/prediction-heatmaps/combined_values.npy')
correlation_grid = np.load('target_0/correlation-heatmaps/correlation_values_pearson.npy')

# View effect size maps
import pandas as pd
effect_map = pd.read_csv('target_0/prediction-effects/effect_size_map_target_0_cluster_1_high_1.csv')
```

**Analysis Methods**:
- **Two-Heatmap Approach**: Separates predictive patterns from manifold topology
- **Effect Size Mapping**: Per-cluster statistical analysis of significant regions  
- **Visualization Integration**: Static PNG heatmaps + interactive HTML cluster maps
